...
3. On the Add backup task page, configure the following settings:
    ...
    | Field | Description |
    | -- | -- |
    | **Backup location** | - **Local path**: Select an external device such as a USB drive, SMB <br>share, or external hard drive.<br>- **Olares Space**: Scan the QR code in **Settings** > your avatar > **Olares<br> Space** via LarePass.<br> - **AWS S3 or Tencent COS**: Click **Add account** in the dialog, or go to <br>**Settings** > **Integrations** > **Link your accounts & data**. |
...
| Action | Description |
| -- | -- |
| **Manage** | - **Edit**: Modify snapshot frequency and time.<br>- **Pause**: Pause the task.<br>- **Delete**: Remove the task and all associated snapshots. |

...
2. 如果你想为一个资料库或文件夹开启同步：

    a. 将鼠标悬停在您想要同步的资料库或文件夹上，点击右侧出现的**操作**图标，点击**同步至本地**。

    ![同步文件至本地](/images/manual/olares/sync-files-local-1.png#bordered)

    b. 在**同步库**弹窗中，设置文件的下载位置，然后点击**确认**，同步随即开始。
    
    完成后，该文件夹图标的左下角会出现一个绿色对勾，表示同步已完成。

3. 如果您想快速定位到本地硬盘上的同步目录，将鼠标悬停在您想要定位的资料库或文件夹上，点击右侧出现的**操作**图标，点击**打开本地同步文件夹**。系统将直接打开该文件夹所在的本地路径。
4. 如果你不再需要同步某个文件夹，将鼠标悬停在该文件夹上，点击右侧出现的**操作**图标，点击**取消同步**。
5. 如果你需要暂时停止数据传输，点击**同步**目录右侧的**暂停**图标，即可一键暂停所有同步任务。

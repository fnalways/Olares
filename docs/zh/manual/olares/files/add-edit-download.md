...
1. 打开**文件管理器**。
2. 根据目录类型，进入分享菜单：
    * **同步**目录：将鼠标悬停在目标文件夹上，点击右侧出现的 **操作**图标，然后点击**共享**。
    * 其他目录：右键点击要分享的目标文件夹，然后选择分享类型。

    ![Share files type](/images/manual/olares/share-files-1.png#bordered)

3. 根据你所选的分享类型，参考下一步中的对应步骤完成相关设置。

#### 第二步：配置分享设置

##### 内部分享

1. 在**邀请用户**中，点击 **+** (加号)，搜索并勾选目标用户或组，点击**邀请**。
2. 在**用户权限设置**列表中，点击用户头像右侧的箭头，为每一位邀请用户分配具体的读写权限，设置完成后，点击**提交**。
3. 点击 **确认**。

##### 公开分享

1. 在**设置密码**中，手动输入访问密码，或点击**重新生成**使用系统随机生成的安全密码。
2. 在**设置有效期**中，选择分享链接过期的具体时间。
3. 其他限制（按需选择）：
    * **限制文件大小**：勾选并填写数值，选择计量单位（如 Mi/Gi/Ti），即可限制访客上传文件的大小。
...
1. 选择是否公开。
2. 如果选择**是**：点击**确认**，则局域网内任何人均可访问。
3. 如果选择**否**，则需要身份验证：
    
    a. 在**邀请用户**中，点击**+**(加号)，选择现有用户或组；或点击**添加用户账户**来手动添加并邀请新用户。
    
    b. 点击**邀请**。
    
    c. 在**用户权限设置**中，为每一位邀请用户配置具体权限，点击**确认**。
...
1. 打开**文件管理器**。
2. 在左侧边栏中，点击**分享**。
3. 在右侧，可以查看所有当前正在分享的文件夹列表。
4. 如果你想查看某个文件夹的详情，右键点击目标文件夹，然后点击**属性**。

    你将看到分享者、原始路径、分享范围、创建者、当前权限、过期时间及链接详情等。

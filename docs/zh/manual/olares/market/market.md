...
| [共享应用](../../../developer/concepts/application.md#共享应用) | 一种特殊类型的社区应用，为集群内所有用户提供统一的、共享的资源或服务。<br>共享应用通过标准 API 或共享入口对外提供服务，集群内任意应用均可直接调用。 |
...
3. 若要取消安装，点击按钮右侧的 <i class="material-symbols-outlined">close_small</i>。
4. 安装完成后，按钮将变为**打开**。
...
此类共享应用仅提供 API 服务，不包含图形界面。无需安装专门的授权应用，任何支持对应 API 的客户端均可直接调用。以 Ollama 为例：

1. 管理员先安装 Ollama。安装后，共享服务在集群内启动，并暴露标准 API 访问入口。
2. 集群成员使用共享服务：  
    - 获取访问入口：共享服务的访问地址可在 Olares **设置** > **应用** > **Ollama** > **入口**中查看。
    - 安装兼容客户端：安装任意支持 Ollama API 的第三方客户端，如 LobeChat、Open WebUI，并在客户端配置中填入共享服务的访问地址。  

#### 自带用户界面的完整应用

此类共享应用同时包含后端服务与 Web 用户界面，可独立为用户提供服务，如 Dify 共享版、ComfyUI 共享版等。
    
1. 管理员首先安装共享应用。此步会启用集群的共享服务，同时也安装一个同名客户端界面作为访问入口。
    ::: tip ComfyUI 启动器
    ComfyUI 共享版包含一个桌面启动器组件，用于管理 ComfyUI 服务和相关资源。管理员需要从 ComfyUI 启动器里配置并启用服务。
    :::
2. 集群成员安装相同的共享版应用。对于这些用户，安装的仅是一个服务访问入口。

...
1. 在克隆之前，确认目标应用支持多实例运行：
    
    a. 打开应用市场，点击应用图标进入详情页。
    
    b. 查看**所需权限**区域。只有包含**多实例**说明的应用才支持克隆功能。
2. 返回应用市场，在左侧导航栏中，点击**我的Olares**。右侧将显示当前已安装的应用列表。
3. 找到目标应用，点击**打开**按钮右侧的下拉箭头，点击**克隆**。
4. 在**克隆应用**弹窗中，完成如下配置，点击**确认**：
    
    a. 为新应用设置一个唯一的名称，避免与现有应用混淆。
    
    b. 设置该应用在桌面上显示的图标名称，便于识别。
5. 在**配置环境变量**弹窗中，根据应用类型完成相应配置，然后点击**确认**。返回应用列表，将看到新克隆的应用，其右侧带有**克隆**标签。
6. 点击**克隆**标签可查看该克隆应用的详情。
